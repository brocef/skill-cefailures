...
- **Capability heading.** A `##` heading per capability, in natural-language imperative form. For API routes that export multiple HTTP methods from one `route.ts`, use the form `## <METHOD>: <action>` (e.g., `## POST: Validate Google ID token and start session`).
...
`<path>#<heading-slug>` — the heading slug is the same one GitHub-flavored markdown renders. Lowercase, words joined by hyphens, punctuation stripped.
