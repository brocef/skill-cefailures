# Broker-mediated product-layer protocol
...
Without coordination, two per-repo agents authoring the "same" capability on web and mobile could drift — same intent, different language, divergent over time. The broker protocol prevents drift by routing product-layer questions through the orchestrator.

## When a per-repo agent should DM the orchestrator
...
If the broker is unreachable (the agent is running outside a brokered session, or the orchestrator agent is not running):
