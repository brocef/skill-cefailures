...
| Bump | Use for |
|------|---------|
| `patch` | Bug fixes, internal refactors, doc-only changes, dependency bumps without behavior change. The default for most work. |
| `minor` | Significant new feature work; new capabilities added in a backward-compatible way. |
| `major` | Breaking changes — removed APIs, incompatible behavior, dropped support. **Only when explicitly instructed by the user.** |

If you're unsure, ask the user before bumping.
