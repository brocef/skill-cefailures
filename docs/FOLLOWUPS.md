...
## Follow-up Items
- ~~**Confirm the fix.** Adding `brocef/skill-cefailures` from <https://claude.ai/code> has not been tested since the change; it needs these commits published first. This is acceptance criterion 9 of `docs/work/.../2026-08-11-make-the-plugin-marketplace-syncable-from-the-claude-ai-web-app` and the only one that did not close locally. If it still fails, the error string will be byte-identical to before — the server collapses every rejection into one message — so a re-test is the only signal.~~ **Completed 2026-08-11** — the maintainer confirmed the sync succeeds against the published repo. Acceptance criterion 9 is closed; GitHub issue #3 was closed on the strength of it.
- ~~**Add the web-app install path to `README.md`** once the above is confirmed. It was deliberately left out of the Installation section: documenting an install route nobody has successfully used is a promise, not a doc.~~ **Completed 2026-08-11** — added to the Installation section; the hedge in `docs/release-notes/v1.17.1.md` was replaced with the confirmed wording at the same time.
