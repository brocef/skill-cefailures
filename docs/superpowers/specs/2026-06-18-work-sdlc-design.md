...
The core mechanic is *moving folders*, so any reference by path breaks on the next transition. The **stable handle is the slug** (`YYYY-MM-DD-<kebab-title>`); references are by slug, and resolution is "find the folder named `<slug>` wherever it now lives." (Jira solved this with `PROJ-123`; we solve it with the slug.) `capabilities.md`'s `Planning doc:` pointer references the slug, not a path.

### A.6 Cross-node initiatives (epics)

One orchestrator work item ("epic") corresponds to many project work items ("tasks") across child nodes — some active, some blocked, some completed.
...
A feature carries both; a refactor is technical-only; a bug references a capability and branches (below). `type` (B.4) becomes a coarse label — "does this item have a product delta?" is the substantive question. This makes "planning relates to capabilities" literal and required, not a convention.
...
**Two pointers bind verb to noun** (each resolves across the storage boundary — see A.11):

- **Forward (noun → verb):** a `Missing` capability's `Planning doc:` pointer holds the realizing work item's **stable ID** (slug).
- **Back (verb → noun):** the work item's `capabilities.yaml` lists the capability files it touches and their intended status transitions.
...
Because `completed` means "no further code changes," the ledger must be accurate before the folder freezes — so `Missing → Supported` is a DoD condition, never a follow-up.
...
- The **worktree invariant** covers the orthogonal case: two agents working *different* items in the *same* repo node would collide on the working tree, so each active item gets its own git worktree. The tool owns this via `work start <slug> --worktree` (creates the worktree + branch, records them in `state.yaml`).
...
| JQL / board view | `rg` / `find` / `tree` over `docs/work/` (generated, never stored) |
...
The model is defined in an abstract vocabulary — **item · status · transition · stable ID · reference · node relation · query · body/fields/attachments**. The filesystem is the *default realization* (Part A describes it concretely), and the `WorkStore` interface (Part B) keeps the model swappable so it can run against an external tracker where one is already in use — the enterprise-portability requirement. Filesystem superpowers — work co-located with code, **one atomic commit landing code change + capability status flip + work-item transition**, grep/diff/PR-review legibility, atomic `mv` as transition — are leveraged as **bonuses on top**, never as load-bearing assumptions the abstract vocabulary can't express.
...
A Python CLI named **`work`** operating on the current node's work store. **Single node only** — no recursion, no skill layer, no migration, no capability *prose* parsing. Delivers: the directory contract, slug management, work-item folders, the two-axis (product/technical) effect record, the legal-transition state machine, the loose DoD gate, basic worktree creation, and the on-demand queries.
...
work new "<title>" [--type T]      → creates backlog/<slug>/ (content.md + state.yaml); prints slug
work list [--status S] [--type T]  the board (reads dir listing + state.yaml)
work show <slug>                   resolve slug→path; print state + body
work path <slug>                   print current path of a slug (the stable-ID resolver)
work start <slug> [--worktree]     inbox|backlog → active/ (optionally make a git worktree+branch)
...
- **`state.yaml`** — machine-readable metadata. Status is **not** stored here (status = the directory / store-status). Fields:
  - `slug` (string, immutable) · `title` · `type` (`feature|bug|refactor|chore|spike`, a coarse label) · `phase` (free-text, e.g. `planning`) · `created` (date) · `worktree` / `branch` (optional, set by `start --worktree`) · `resolution` (set on completion: `done|wontfix|duplicate|superseded`) · `dod` (optional acknowledged-checklist record).
- **`capabilities.yaml`** — the machine-readable **back-pointer** for the product axis: the capability files this item touches and their intended status transitions (e.g. `{file: …, heading: …, from: Missing, to: Supported}`). Drives traceability, pointer resolution, and the `complete` DoD check (B.6). The tool reads these *pointers*, never the capability *prose* (mechanism vs. judgment). Empty/absent for technical-only items.
...
**Slug rules:** `YYYY-MM-DD-<kebab-title>` from the creation date + title. Uniqueness is enforced **within the node** (collision appends `-2`, `-3`, …). The slug is immutable once created and is the only cross-reference handle.

### B.5 Command behavior

- **`init`** — create the five status directories (idempotent). Refuse outside a git repo (suggest `git init`).
- **`new`** — generate the slug, create `backlog/<slug>/` with `content.md` (seeded from the title, the `## Product changes` / `## Technical changes` scaffold, and any piped stdin) and `state.yaml` (`type` default `feature`). Print the slug.
- **`list`** — walk the status directories, read each `state.yaml`, print a table (slug · status=dir · type · phase · title). `--status` / `--type` filter.
- **`show` / `path`** — resolve the slug to its current folder anywhere under `docs/work/`; `show` prints `state.yaml` + the head of `content.md`; `path` prints just the resolved path (for scripting/references).
- **`start`** — move `inbox|backlog → active`. With `--worktree`, run `git worktree add` on a new branch and record `worktree`/`branch` in `state.yaml`.
- **`block`** — move `active → blocked`; append the `--on` target to `links.yaml` `blocked_on`.
- **`unblock`** — move `blocked → active`; **refuse** if any `blocked_on` entry is unresolved (a referenced slug not in `completed/`, or an `external` entry still present).
- **`complete`** — the DoD gate (B.6); on pass, set `resolution`, move `active → completed`.
- **`drop`** — `git rm -r` the folder (only from `inbox`/`backlog`).
...
1. Read the node's DoD checklist (a default set, overridable by `docs/work/dod.yaml`). Default items: *tests pass · docs synced · capabilities reconciled (the `capabilities.yaml` transitions applied to the ledger — A.7) · reviewed · version offered*.
...
- Every transition performs `git mv` (or `git rm` for `drop`) and **stages** the change. A `--commit` flag additionally commits with a conventional message (`work: <verb> <slug>`); default is stage-only so the agent controls commit boundaries. *(Default is an open question — see B.9.)*
- **Node detection:** walk up from cwd to the nearest directory containing `docs/work/`; operate there. If none is found, error and suggest `work init`.

### B.8 Testing

pytest over `tmp_path` git repos: slug generation + collision suffixing + immutability; every legal transition and a representative set of refused illegal ones; `completed/` is a sink; `unblock` refusal on an unresolved blocker; `list` filters; `complete` refusal without `--confirm`; slug→path resolution *after* the folder has moved; `--worktree` creates and records a worktree.

### B.9 Open questions (refinement backlog — deliberate, not placeholders)

- `--commit` default: auto-commit each transition (clean audit trail, many small commits) vs. stage-only (agent bundles commits).
- DoD checklist source of truth: `docs/work/dod.yaml` vs. a section in `AGENTS.md` vs. tool default only.
- Where `new` lands by default: `backlog/` (chosen) vs. `inbox/`.
- `content.md` seeding: plain title vs. a template; whether `new` also seeds an empty `spec.md`.
- Exact `type` enum and whether `epic` belongs here or only appears in Spec 2 — and whether `type` still earns its place now that the product/technical axis (A.7) carries the substantive classification.
- `phase` free-text vs. a fixed enum.
- Whether basic worktree creation belongs in Spec 1 (currently yes) or Spec 2.
...
2. **Spec 2 — Cross-node / recursion:** node discovery, epics, `initiative:` back-pointers, `reconcile` (scan children → consolidated rollup), escalate/delegate via inbox, and the two-layer × two-layer capability mapping (epic ↔ product-layer ledger, task ↔ co-located ledger; product-layer coordination over the inbox channel).
