...
- `--burst-window 0` exits as soon as the first arrival has been delivered. If the first "arrival" was a multi-line backlog at startup, all of it is emitted in one go (no waiting between lines) before exit — the `0` does not split a backlog. Note this is "no further waiting," not transactional atomicity: the per-batch cursor-advance rule still applies (see SIGINT case below).
- Stdout is the existing display-format lines, in arrival order. Cursor advances per inbox-read batch (matching today's `broker follow`): all lines from a single `inbox.read_from` call are emitted, then the cursor is advanced once for the whole batch. Implication: if `broker recv` is killed (SIGINT, SIGKILL) between the print loop and the cursor write, lines that were emitted but not yet committed will be re-emitted on the next `broker recv` — the user-visible contract is "no messages lost," not "no messages re-emitted." The window for this is microseconds.
...
**`broker recv` killed mid-burst-window.** SIGINT or SIGKILL during the burst window may cause re-emission at the per-batch granularity: a batch's lines are emitted to stdout first, then the cursor is advanced once for the batch, so a kill landing between print and cursor-write leaves those lines in the backlog. The next `broker recv` will re-emit them. Combined with on-disk persistence, this means no messages are *lost*; some may simply be received twice (once before the kill, once after). The agent's authority/idempotency rules apply when handling re-emitted messages.
