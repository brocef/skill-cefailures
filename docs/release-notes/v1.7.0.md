...
- The README now enumerates all eight skills with one-line descriptions, documents the `/broker-mode` slash command, and surfaces previously-undocumented plugin capabilities. The "Creating a new skill" instructions correctly identify OpenAI as the default backend, with explicit examples for the Anthropic SDK and local Claude CLI options.
