...
- Broker health check Check 4 now accepts either `Bash(broker:*)` (tool-name form) or `Bash(broker *)` (command-prefix form) as a valid permission entry. Previously only the colon form passed.
